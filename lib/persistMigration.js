...
module.exports = async function migrateLegacyPersist(persistDir, storage, log) {
	let files
	try {
		files = await fs.readdir(persistDir)
	} catch (err) {
		if (err.code !== 'ENOENT' && log && log.debug) log.debug(`persistMigration readdir failed: ${err.message}`)
		return
	}

	for (const file of files) {
		if (!/^[a-f0-9]{32}$/.test(file)) continue
		const filePath = path.join(persistDir, file)
		try {
			const raw = await fs.readFile(filePath, 'utf8')
			const datum = JSON.parse(raw)
			if (!datum || typeof datum !== 'object' || typeof datum.key !== 'string' || !('value' in datum)) continue
			const expectedMd5 = crypto.createHash('md5').update(datum.key).digest('hex')
			if (expectedMd5 !== file) continue
